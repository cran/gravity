...
#' @export 
#' 
Tetrads <- function(y, dist, x, k="USA", ell="JPN", multiway_vcov=TRUE, 
                    data, ...){
  if(!is.data.frame(data))stop("'data' must be a 'data.frame'")
  if((multiway_vcov %in% c(TRUE, FALSE)) == FALSE){
    stop("'multiway_vcov' has to be either 'TRUE' or 'FALSE'")}
  ...
  for(i in unique(d_3$iso_o)){
    ...
    d_3[d_3$iso_o==i,]$lXinratk <- d_3[d_3$iso_o==i,]$y_log - 
      d_3[d_3$iso_o==i & d_3$iso_d==k,]$y_log
    
    d_3[d_3$iso_o==i,]$ldistratk <- d_3[d_3$iso_o==i,]$dist_log - 
      d_3[d_3$iso_o==i & d_3$iso_d==k,]$dist_log
  }
  ...
  for(j in 1:num.ind.var){
    for(i in unique(d_3$iso_o)){
      ...
      ind.var.ratk[[j]][ind.var.ratk[[num.ind.var+1]]==i] <- 
        ((d_3[d_3$iso_o==i,])[x[j]])[,1] - 
        (d_3[d_3$iso_o==i & d_3$iso_d==k,])[x[j]][,1]
    }
  }
  ...
  for(i in unique(d_3$iso_d)){
    ...
    d_3[d_3$iso_d==i,]$lXinrat <- d_3[d_3$iso_d==i,]$lXinratk - 
      d_3[d_3$iso_d==i & d_3$iso_o==ell,]$lXinratk
    
    d_3[d_3$iso_d==i,]$ldistrat <- d_3[d_3$iso_d==i,]$ldistratk - 
      d_3[d_3$iso_d==i & d_3$iso_o==ell,]$ldistratk
  }
  ...
  for(j in 1:num.ind.var){
    for(i in unique(d_3$iso_d)){
      ...
      ind.var.rat[[j]][ind.var.rat[[num.ind.var+1]]==i] <- 
        ((d_3[d_3$iso_d==i,])[x[j]])[,1] - 
        (d_3[d_3$iso_d==i & d_3$iso_o==ell,])[x[j]][,1]
    }
  }
  ...
  model.Tetrads_vcov <- multiwayvcov::cluster.vcov(
    model=model.Tetrads, cluster = cluster.formula)

  model.Tetrads.robust <- lmtest::coeftest(
    x=model.Tetrads, vcov=model.Tetrads_vcov)
  
  # Return ---------------------------------------------------------------------
  
  if(multiway_vcov == TRUE){
    summary.Ted.1 <- .robustsummary.lm(model.Tetrads, robust=TRUE)
    summary.Ted.1$coefficients <- 
      model.Tetrads.robust[1:length(rownames(model.Tetrads.robust)),]
