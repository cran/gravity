...
#' 
#' @details \code{PPML} is an estimation method for gravity models presented
...
#' excluded from the dataset. Unlike the other functions, flows equal to zero 
...
#' In contrast to the other functions, trade flows equal to zero do not have
#' to be excluded when using \code{PPML}.
...
PPML <- function(y, dist, x, vce_robust=TRUE, data, ...){
  ...
  model.PPML <- stats::glm(form2, data = d, family = stats::quasipoisson)
  model.PPML.robust <- lmtest::coeftest(
    model.PPML, vcov=sandwich::vcovHC(model.PPML, "HC1"))
  
  # Return --------------------------------------------------------------------- 
  
  if(vce_robust == TRUE){
    summary.PPML.1 <- .robustsummary.lm(model.PPML, robust=TRUE)
    summary.PPML.1$coefficients <- 
      model.PPML.robust[1:length(rownames(model.PPML.robust)),]
